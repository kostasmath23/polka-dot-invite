      ...
      <div className="max-w-screen-xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-20 md:gap-24 items-start">
        <div className="flex flex-col items-center text-center gap-8">
          ...
          <div className="relative overflow-hidden rounded-xl shadow-xl w-full max-w-md invitation-glow">
            <img src="/images/invite.jpg" alt={t.invitation.invitationAlt} className="w-full h-auto object-cover" />
          </div>
        </div>
            ...
            {banks.map((item, i) => (
              <div key={i} className="bg-gray-100 rounded-xl p-6 shadow-md">
                ...
                <div className="flex items-center justify-between gap-4 flex-wrap">
                  ...
                  <button onClick={() => handleCopy(item.iban)} className="text-blue-600 hover:text-blue-800 active:scale-95 transition" title={t.invitation.copy} aria-label={t.invitation.copy}>
                    <FaRegCopy size={22} />
                  </button>
                </div>
              </div>
